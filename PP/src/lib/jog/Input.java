...
public abstract class Input {
	...
	public static void update(EventHandler handler) {
		while (Mouse.next()) {
			if (Mouse.getEventButtonState()) {
				if (Mouse.getEventButton() >= 0) {
					handler.mousePressed(Mouse.getEventButton(), getMouseX(), getMouseY());
				}
			} 
			else {
				if (Mouse.getEventButton() >= 0) {
					handler.mouseReleased(Mouse.getEventButton(), getMouseX(), getMouseY());
				}
			}
		}
		while (Keyboard.next()) {
		...
		if (mouseScroll < 0) {
			handler.mouseReleased(MOUSE_WHEEL_DOWN, getMouseX(), getMouseY());
		} 
		else if (mouseScroll > 0) {
			handler.mouseReleased(MOUSE_WHEEL_UP, getMouseX(), getMouseY());
		}
	}
	...
	public static boolean isMouseInRect(int x_coordinate, int y_coordinate, int width, int height) {
		return (getMouseX() >= x_coordinate && getMouseX() <= x_coordinate + width && getMouseY() >= y_coordinate && getMouseY() <= y_coordinate + height);
	}
