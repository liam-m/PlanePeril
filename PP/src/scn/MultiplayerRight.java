...
public class MultiplayerRight extends Multiplayer {
	...
	public void update(double dt) {
		...
		if (selected_aircraft != null) {
			...
			// allows to take control by just pressing right/right or A/D
	...
	public void mousePressed(int key, int x, int y) {
		...
		if (key == input.MOUSE_LEFT) {
			...
			if (selected_aircraft != null) {
				...
				for (Waypoint w : right_waypoints) {
	...
	public void mouseReleased(int key, int x, int y) {
		...
		if (key == input.MOUSE_LEFT && right_airport.isMouseOver(x - 16, y - 16)) {
			// must wait at least 5 seconds between aircraft takeoff
			if (next_take_off - timer <= 0) {
				try {
					right_airport.takeoff();
					generateFlight(true);
	...
	private java.util.ArrayList<Waypoint> getAvailableEntryPoints() {
		...
		for (Waypoint entry_point : right_entryexit_waypoints) {
	...
	private java.util.ArrayList<Integer> getIdAvailableEntryPointsAltitudes() {
		...
		for (Waypoint entry_point : right_entryexit_waypoints) {
			base_id++;
			if (entry_point != right_airport) { // Airport is excluded as it is said initially whether it should come from airport or elsewhere
	...
	private Aircraft createAircraft(boolean fromAirport) {
		...
		int destination = RandomNumber.randInclusiveInt(0, right_entryexit_waypoints.length - 1);
		...
		if (fromAirport) {
			...
			if (available_origins.isEmpty()) { // Creates a plane in waypoint with planes of different altitude than that of the new plane.
				...
				origin_point = right_entryexit_waypoints[origin]; 
				...
				for (int i = 0; i < right_entryexit_waypoints.length; i++) { // getting id for an entry point
					if (right_entryexit_waypoints[i].equals(origin_point)) {
						origin = i;
						break;
					}
				}
		...
		while (right_entryexit_waypoints[destination].equals(right_entryexit_waypoints[origin]) || fromAirport && right_entryexit_waypoints[destination] instanceof Airport) {
			destination = RandomNumber.randInclusiveInt(0, right_entryexit_waypoints.length - 1);
		}			
		
		Waypoint destination_point = right_entryexit_waypoints[destination];
		// Name
		String name = "";
		...
		return new Aircraft(name, aircraft_image, 32 + (int) (10 * Math.random()), 1, new FlightPlan(origin_point, 
				destination_point, this.right_waypoints, this.right_holding_waypoints, right_airport_takeoff_waypoint), preferred_altitude_index);
	}
